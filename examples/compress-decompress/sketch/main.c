#define THE_FOX "The quick brown fox jumps over the lazy dog."
/*
...
void handleIncomingMessage() {
  char stx = Serial.read();
  if (stx == 0x2) {
    Serial.setTimeout(500);
    int len = (int)Serial.read();
    char msg[len];
    Serial.readBytes((char*)&msg, len);
    char etx = Serial.read();
    if (etx == 0x3) {
  		char raw[sizeof(THE_FOX) + 1];
      	memset(raw, 0, sizeof(raw));
  		unishox2_decompress_simple((char const*)&msg, sizeof(raw), (char*)&raw);
      	if(strcmp(raw, THE_FOX) ==0){
          Serial.println(msg);
        }
    }
  }  
...
void sendInitialMessage() {
  char raw[] = THE_FOX;
  char compressed[sizeof(raw)];
  unishox2_compress_simple((char const*)&raw, sizeof(raw), (char*)&compressed);  
  Serial.println((char*)&compressed);
}
